...
void ArenaEndTemp(M_ArenaTemp temp) {
    ArenaDeallocTo(temp.arena, temp.pos);
}
...
void PoolInit(M_Pool* pool, u64 element_size) {
	...
	pool->memory = mmap(NULL, M_POOL_MAX, PROT_NONE, MAP_PRIVATE | MAP_ANON, -1, 0);
	pool->max = M_POOL_MAX;
	pool->commit_position = 0;
	pool->element_size = AlignForwardU64(element_size, DEFAULT_ALIGNMENT);
	pool->head = NULL;
}
...
void* PoolAlloc(M_Pool* pool) {
	if (pool->head) {
		...
		mprotect(commit_ptr, M_POOL_COMMIT_CHUNK * pool->element_size, PROT_READ | PROT_WRITE);
		PoolDeallocRange(pool, commit_ptr, M_POOL_COMMIT_CHUNK);
